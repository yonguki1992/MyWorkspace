...
public class StreamEx5 {
	...
	public static void main(String[] args) {
		...
		dice.map(i-> Stream.of(1,2,3,4,5,6).map(i2->new int[] {i,i2}) ).forEach(i3 -> i3.forEach(i4 -> System.out.println(Arrays.toString(i4))));

		//map
		Stream<Integer> dice2 = IntStream.rangeClosed(1, 6).boxed();
		Stream<Stream<int[]>> diceStreamStream = dice2.map(i-> Stream.of(1,2,3,4,5,6).map(i2->new int[] {i,i2}));
		
		//flatmap
		System.out.println("flatmap");
		Stream<Integer> dice3 = IntStream.rangeClosed(1, 6).boxed();
		Stream<int[]> diceStream = dice3.flatMap(i-> Stream.of(1,2,3,4,5,6).map(i2->new int[] {i,i2}));
